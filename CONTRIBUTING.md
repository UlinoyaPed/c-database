...
项目对外或跨文件使用的自定义函数应使用统一的项目或模块前缀，类似 OpenGL 的命名方式，通过前缀避免与其他库或模块的函数发生命名冲突。例如项目缩写为 `CDB` 时，可以使用 `CDBLoadFile()`、`CDBCreateStudent()`、`CDBSaveRecord()`。不同模块可以继续在前缀后体现功能含义。仅在单个 `.c` 文件内部使用的辅助函数应声明为 `static`，避免向外暴露符号。
